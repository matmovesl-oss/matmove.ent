...
import { 
  Car, Package, Truck, Navigation, MapPin, ShieldCheck, 
  CreditCard, Smartphone, X, Wallet, Bell 
} from 'lucide-react';

export function RiderDashboard({ profile, wallet, onOpenBooking, onOpenTopUp }: any) {
  const [selectedVehicle, setSelectedVehicle] = useState<'car' | 'keke' | 'bike' | 'truck'>('car');
    ...
    <div className="flex-1 bg-slate-50 min-h-screen">
      ...
      <header className="bg-white border-b border-slate-200 px-8 py-4 flex justify-between items-center sticky top-0 z-10">
        ...
        <div className="flex items-center gap-6">
          ...
          <div className="flex items-center gap-2">
            ...
            <span className="text-sm font-bold text-slate-800">{profile?.full_name || 'User'}</span>
          </div>
        </div>
      </header>
      ...
      <div className="p-8 max-w-6xl mx-auto space-y-8">
        ...
        <div className="flex justify-between items-end">
          <div>
            <h1 className="text-3xl font-bold text-slate-900">Where to, {profile?.full_name?.split(' ')[0] || 'Rider'}? 👋</h1>
        ...
        <div className="grid grid-cols-3 gap-6">
          ...
          <div className="bg-white rounded-3xl p-6 border border-slate-200 shadow-sm flex flex-col justify-center">
            ...
            <p className="text-slate-500 text-xs mt-1">Verified drivers with GPS journey tracking and automated wallet settlement.</p>
          </div>
        </div>

        {/* Uber-Style Ride Request & Map Panel */}
        <div className="grid grid-cols-3 gap-8 bg-white p-6 rounded-3xl border border-slate-200 shadow-sm">
          <div className="col-span-1 space-y-4">
              ...
              <div className="grid grid-cols-2 gap-2 mt-2">
                {vehicleOptions.map((v) => (
                  <button
                    ...
                    className={`p-3 rounded-xl border text-left transition ${selectedVehicle === v.id ? 'border-blue-600 bg-blue-50/50 ring-2 ring-blue-600/20' : 'border-slate-200 hover:border-slate-300'}`}
                  >
                    <v.icon size={18} className={selectedVehicle === v.id ? 'text-blue-600' : 'text-slate-500'} />
            ...
            {/* Live Read-Only Authoritative Price */}
            {fareEstimate && (
              <div className="p-4 bg-slate-900 text-white rounded-2xl flex justify-between items-center">
                <div>
                  ...
                  <span className="text-2xl font-bold text-emerald-400">SLE {fareEstimate}</span>
                </div>
                <span className="text-xs bg-white/10 px-2 py-1 rounded text-slate-300 font-semibold">Read-Only</span>
              </div>
            )}
            ...
            >
              Confirm {vehicleOptions.find(v => v.id === selectedVehicle)?.name} Request
            </button>
          </div>
          ...
          <div className="col-span-2 bg-slate-100 rounded-2xl relative overflow-hidden border border-slate-200 min-h-[380px] flex items-center justify-center">
            <iframe
              title="Sierra Leone Interactive Map"
              width="100%"
              height="100%"
        ...
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-3xl max-w-md w-full p-6 shadow-2xl relative">
            ...
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-3">
                {[
                  ...
                  <div
                    ...
                    className={`p-3 border rounded-xl cursor-pointer transition flex items-center gap-3 ${provider === p.id ? 'border-blue-600 bg-blue-50/50 ring-2 ring-blue-600/20' : 'border-slate-200 hover:border-slate-300'}`}
                  >
                    <div className={`p-2 rounded-lg ${p.bg} ${p.color}`}><p.icon size={18} /></div>
              ...
              {(provider === 'orange' || provider === 'africell') && (
                <div>
                  <label className="block text-xs font-bold text-slate-700 uppercase mb-1">Mobile Money Number</label>
                  <input 
                    type="text" 
              ...
              <button 
                onClick={() => {
                  alert(`Processing SLE ${topUpAmount || '0'} top-up via ${provider === 'vult' ? 'VULT PAY' : provider === 'flot' ? 'FLOT PAY' : provider.toUpperCase()}`);
                  setIsTopUpModalOpen(false);
                }} 
                className="w-full bg-blue-600 text-white font-bold p-3.5 rounded-xl hover:bg-blue-700 transition mt-4"
              >
                Pay via {provider === 'vult' ? 'VULT PAY' : provider === 'flot' ? 'FLOT PAY' : provider.toUpperCase()}
              </button>
            </div>
          </div>
        </div>
