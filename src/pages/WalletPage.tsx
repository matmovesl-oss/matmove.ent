...
export function WalletPage({ profile, wallet, onClose }: any) {
  ...
  const closeModals = () => {
    ...
    setIsProcessing(false); // Resets the stuck spinner
    setAmount('');
    setLoadMethod(null);
  };
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 py-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
        ...
        <div onClick={handleOpenWithdraw} className="bg-white border border-slate-200 rounded-3xl p-6 shadow-sm hover:border-emerald-500 cursor-pointer transition flex flex-col justify-between">
          <div className="flex items-center gap-3 mb-4"><div className="w-12 h-12 bg-emerald-50 text-emerald-600 rounded-2xl flex items-center justify-center"><ArrowUpRight size={24} /></div><div><h3 className="font-bold text-slate-900 text-base">Withdraw SLE</h3><p className="text-xs text-slate-500">Transfer funds to Mobile Money.</p></div></div>
          <button className="w-full bg-emerald-600 text-white font-bold py-3 rounded-xl hover:bg-emerald-700 transition text-sm">Withdraw Funds</button>
        </div>
      </div>
      ...
      {isLoadModalOpen && (
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-3xl p-8 max-w-md w-full relative shadow-2xl">
            ...
            <button onClick={executeLoad} disabled={isProcessing || !amount || !loadMethod} className="w-full bg-slate-900 text-white font-bold p-4 rounded-xl flex justify-center gap-2 disabled:opacity-50">{isProcessing ? <Loader2 className="animate-spin" size={20} /> : 'Proceed to Checkout'}</button>
          </div>
        </div>
      )}
      ...
      {isWithdrawModalOpen && (
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-3xl p-8 max-w-md w-full relative shadow-2xl">
            ...
            <button onClick={executeWithdraw} disabled={isProcessing || !amount} className="w-full bg-emerald-600 text-white font-bold p-4 rounded-xl flex justify-center gap-2 disabled:opacity-50">{isProcessing ? <Loader2 className="animate-spin" size={20} /> : 'Confirm Cashout'}</button>
          </div>
        </div>
      )}
    </div>
