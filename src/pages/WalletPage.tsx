...
export function WalletPage({ profile, wallet, onClose }: any) {
  ...
  // Riders are automatically approved to withdraw remaining funds.
  ...
  const closeModals = () => {
    ...
    setIsProcessing(false); // Fixes the stuck spinner issue
    setAmount('');
    setLoadMethod(null);
  };
  ...
  const executeLoad = async () => {
    ...
    try {
      ...
      const res = await fetch(endpoint, {
        ...
        body: JSON.stringify({ amount, userId: profile.id, email: profile.email, phone: profile.phone, role: profile.role })
      });
